...
if [[ "$PORT" != "" ]]; then
  if [[ "$PORTFORWARD" != "" ]]; then
    export POD_NAME=$(kubectl get pods --namespace "$NAMESPACE" -l "app.kubernetes.io/name=$SERVICE,app.kubernetes.io/instance=$SERVICE-release" -o jsonpath="{.items[0].metadata.name}"|  head -n 1)
    kubectl -n "$NAMESPACE" port-forward "$POD_NAME" "$PORTFORWARD:$PORT" >/dev/null 2>&1 &
    PID=$!
    echo "$SERVICE port-forward PID is $PID"
  fi
fi
