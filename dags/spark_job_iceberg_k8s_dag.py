...
@dag(
    dag_id='spark_job_iceberg_k8s',
    description='Spark/Iceberg job using Airflow 3 with k8s',
#    schedule=timedelta(hours=1),
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=['spark', 's3', 'iceberg', 'k8s'],
...
def spark_job_dag():
    ...
    def create_template() -> dict:
        return (
            ...
            .override_with("iceberg_spark.py",
               {
                    "SPARK_JOB_NAME": "Iceberg Spark Job",
                    "DB_NAME": config_manager.catalog_config.catalog_name,
                    "TABLE_NAME": "mytable"
                }
           )
        )
